...
namespace CommonUtilities
{
	...
	public:
		...
		bool UpdateEvents(const UINT& aMessage, const WPARAM& aWParam, const LPARAM& anLParam, const HWND& aWindowHandle);
		...
		void SetMouseClientPosition(const HWND& aWindowHandle, const POINT& aPoint);
		void LockCursorToClient(const HWND& aWindowHandle);
		POINT GetMouseScreenPosition() const;
		POINT GetMouseClientPosition() const;
		POINT GetDeltaMouseMove() const;
		int GetDeltaMouseScroll() const;

	private:
		std::array<bool, 256> myCurrentState;
		std::array<bool, 256> myIntermediateState;
		std::array<bool, 256> myPreviousState;
		POINT myMouseDelta;
		POINT myPendingMouseDelta;
		//POINT myPreviousMouseScreenPosition;
		POINT myMouseClientPosition;
		int myScrollDeltaDistance;
		int myToggleLockCursorToClientButton;
		bool myShouldLockcursorToClient;
	};
